...
.auth-wrap {
  ...
  padding: 24px;
  background:
    radial-gradient(1200px 600px at 50% -10%, color-mix(in srgb, var(--c-up-soft) 40%, transparent), transparent),
    var(--c-bg);
}
.auth-card {
  width: 100%;
...
.auth-card .brand { font-size: 26px; font-weight: 600; letter-spacing: -0.02em; color: var(--c-text); }
.auth-card .sub { font-size: 13px; color: var(--c-text-mute); margin-top: 4px; }
.auth-card .tabs {
...
.auth-card input:focus { outline: none; border-color: var(--c-text-soft); }
.auth-card .submit { margin-top: 6px; padding: 11px 0; font-size: 14px; font-weight: 600; }
.auth-card .hint { font-size: 12px; color: var(--c-text-mute); margin-top: 4px; line-height: 1.5; }
.auth-card .back { display: inline-block; margin-top: 18px; font-size: 13px; color: var(--c-text-mute); }
